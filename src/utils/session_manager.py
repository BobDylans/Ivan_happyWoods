...
统一的会话管理器：内存缓存 + 数据库持久化（可选）

特性：
- 内存缓存：快速读取热数据
- 数据库持久化：永久存储，支持横向扩展（可选）
- 自动降级：数据库不可用时自动降级为纯内存模式
- 统计监控：缓存命中率、数据库读写统计

向后兼容：
- SessionHistoryManager = HybridSessionManager (别名)
"""
...
class HybridSessionManager:
    ...
    def __init__(
        ...
        Args:
            ...
            max_history: (兼容参数) 等同于 memory_limit
            ttl: (兼容参数) 等同于 ttl_hours
        """
        # 兼容旧版本参数
        ...
        self.max_history = memory_limit  # 兼容旧版本
        self._ttl_hours = ttl_hours
        self.ttl = timedelta(hours=ttl_hours)  # 兼容旧版本
        ...
        if self._fallback_mode:
            logger.warning("⚠️ SessionManager 运行在纯内存模式（数据库未启用）")
        else:
            logger.info(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            ...
            "total_sessions": len(self._sessions),  # 兼容旧版本
            "total_messages": sum(len(msgs) for msgs in self._sessions.values()),  # 兼容旧版本
...
# SessionHistoryManager 是 HybridSessionManager 的别名
# 保持与旧代码的兼容性
SessionHistoryManager = HybridSessionManager
